...
# Points to: https://github.com/scottmcrae/Emotions/tree/main/images
GITHUB_RAW = "https://raw.githubusercontent.com/scottmcrae/Emotions/main/images"

# ── Emotion definitions ───────────────────────────────────────────────────────
# Filenames match the images folder in scottmcrae/Emotions (e.g. Anger.png)
EMOTIONS = [
...
st.markdown('<div class="round-label">identify the emotion</div>', unsafe_allow_html=True)
...
try:
    ...
    if st.session_state.wrong_attempts > 0 and not st.session_state.correct:
        ...
        st.image(display_bytes, width=240)
    ...
    st.warning(
        f"⚠️ Could not load image for **{target['label']}**. "
        f"Make sure `{target['label']}.png` is uploaded to your "
        f"[GitHub repo → images/ folder](https://github.com/scottmcrae/Emotions/tree/main/images)."
    )

# Correct answer feedback
if st.session_state.answered and st.session_state.correct:
    st.markdown(
        f'<div class="feedback-correct">✅ Correct! That\'s <em>{target["label"]}</em> {target["emoji"]}</div>',
        unsafe_allow_html=True
    )
