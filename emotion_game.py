...
# Image — base64 inline so no Streamlit gap from title
if img_bytes:
    ...
    correct_msg = f'<div class="feedback-correct">✅ Correct! That\'s <em>{target["label"]}</em>!</div>' if st.session_state.answered and st.session_state.correct else ""
    st.markdown(
        f'<img style="width:100%;border-radius:16px;display:block;margin-bottom:0.75rem;" src="data:image/jpeg;base64,{b64}" />'
        f'{try_again}{correct_msg}',
        unsafe_allow_html=True
    )
else:
    st.error("⚠️ Image not found.")

# Buttons — st.button so they actually work
