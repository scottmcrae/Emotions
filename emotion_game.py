...
/* On small screens: score chips move above image, side by side */
...
<span style="color:#FF0000">E</span><span style="color:#FF7700">m</span><span style="color:#FFD700">o</span><span style="color:#00AA00">t</span><span style="color:#0000FF">i</span><span style="color:#4B0082">o</span><span style="color:#8B00FF">n</span><span style="color:#1a1a2e"> </span><span style="color:#FF0000">M</span><span style="color:#FF7700">a</span><span style="color:#FFD700">t</span><span style="color:#00AA00">c</span><span style="color:#0000FF">h</span><span style="color:#1a1a2e">!</span>
</div>
""", unsafe_allow_html=True)

# Score chips — on desktop: right of image. On mobile: above image as a row
st.markdown(f"""
<div class="score-section" style="display:none" id="mobile-scores">
  <span class="score-chip">⭐ Score<br>{st.session_state.score}</span>
  <span class="score-chip">🎯 Round<br>{st.session_state.round}</span>
</div>
""", unsafe_allow_html=True)

# ── Image + desktop score side by side ───────────────────────────────────────
...
with score_col:
    st.markdown(f"""
<div style="display:flex; flex-direction:column; gap:12px; padding-left:20px; margin-top:1.5rem;">
  <span class="score-chip">⭐ Score<br>{st.session_state.score}</span>
  <span class="score-chip">🎯 Round<br>{st.session_state.round}</span>
</div>
<style>
  /* On mobile, hide the Streamlit score_col and show mobile-scores instead */
  @media (max-width: 600px) {{
    #mobile-scores {{ display: flex !important; flex-direction: row; gap: 12px; margin-bottom: 0.75rem; }}
  }}
</style>
""", unsafe_allow_html=True)
