...
hr { border: none; border-top: 1px solid #e5e0d8; margin: 0.5rem 0; }
</style>
...
if img_bytes is not None:
    if st.session_state.wrong_attempts > 0 and not st.session_state.correct:
        ...
        try_again = '<div class="feedback-wrong">🔄 Try Again</div>'
    else:
        disp = img_bytes
        try_again = ""
    b64 = base64.b64encode(disp).decode()
    img_tag = f'<img class="game-img" src="data:image/jpeg;base64,{b64}" />'
else:
    img_tag = '<p style="color:red">⚠️ Image not found in GitHub repo.</p>'
    try_again = ""

# Everything in ONE markdown call — title + image, no Streamlit gap possible
st.markdown(f"""
<div class="game-title"><span style="color:#FF0000">E</span><span style="color:#FF7700">m</span><span style="color:#FFD700">o</span><span style="color:#00AA00">t</span><span style="color:#0000FF">i</span><span style="color:#4B0082">o</span><span style="color:#8B00FF">n</span>&nbsp;<span style="color:#FF0000">M</span><span style="color:#FF7700">a</span><span style="color:#FFD700">t</span><span style="color:#00AA00">c</span><span style="color:#0000FF">h</span><span style="color:#1a1a2e">!</span></div>{img_tag}{try_again}
""", unsafe_allow_html=True)
